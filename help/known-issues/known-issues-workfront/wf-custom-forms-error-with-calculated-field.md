---
title: 'カスタムフォーム：計算フィールドの設定時にエラーが発生しました'
description: ユーザーがカスタムフォームで計算フィールドを作成または編集する際、計算フィールドの式にカスタムフィールドを含めると、式は無効と見なされます。「保存」ボタンは無効になっており、ユーザーはカスタムフィールドから移動できません。また、ユーザーには、フィールドの下に Whoops メッセージが表示されます。
hidefromtoc: true
