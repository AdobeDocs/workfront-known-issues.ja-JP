...
### 2024 年 10 月 3 日のメンテナンス更新

#### ボード

**アーカイブした接続カードが同期されない**

パフォーマンスの問題に対処するために、アーカイブされた接続カードは同期されなくなります。 Workfrontのタスクまたは問題に加えた変更は、アーカイブされたカードには反映されません。 カードを復元すると、カードは再び同期されます。
...
**プルーフに SSO を使用する場合、電子署名は追加できません**

SSO を使用して Proof にログインする場合、ユーザーは電子署名を必要とするプルーフを設定できません。
...
**スクラムチームを割り当てる際に「イテレーションに追加」オプションが 2 回表示される**

アジャイルスクラムチームにタスクまたは問題を割り当てると、その他メニューに「イテレーションに追加」オプションが 2 回表示されます。この問題は、チームを割り当てる機能には影響せず、スクラム以外のアジャイルチームには表示されません。

#### カスタムフォーム

**計算エディターのフィールドのリストが 200 項目に制限されている**

カスタムフォームの計算フィールドの計算エディターで、オブジェクトのフィールドリストがシステムのパフォーマンスを向上させるために 200 項目に制限されるようになりました。 フィールド名がわかっている場合は、リストをスクロールする代わりに、typeahead オプションを使用してフィールド名を検索できます。
