...
表示ロジックを使用してユーザーがカスタムフォームに情報を入力すると、表示ロジックに基づいて表示されるフィールドは表示されません。

#### ホーム

**投稿者に、機能しない「作業をする」ボタンが表示されました**

投稿者ユーザーは、ホームの自分の作業ウィジェットにある機能しない「作業」ボタンにアクセスできます。

#### レポート

**カスタム四半期は報告書プロンプトに表示されません**

ユーザーが日付ベースのプロンプトを使用してレポートを実行する場合、カスタマイズされた四半期はプロンプトの選択リストに表示されません。
...
**「削除へのアクセス」エラーが原因で、プロジェクトを共有できない**
...
**カテゴリ名フィールドがカスタムフォームにリンクされない**

カテゴリ名フィールドを含むレポートでリストを表示している場合、カテゴリ名列のコンテンツをクリックして、対応するカスタムフォームを開くことができません。

**レポートでカスタムカラーのグループ化が機能しない**

Report Builder でグループにカスタムカラーを割り当てると、レポートの実行時にカラーの選択が表示されません。
...
レポートの実行時に、プロンプトとしてカスタムの Date/Time フィールドを使用すると、プロンプトインターフェイスに時間選択フィールドが表示されません。

**共有レポート/ダッシュボードを表示できない**

ユーザーが、共有されているレポートまたはダッシュボードにアクセスしようとすると、レポートまたはダッシュボードにアクセスできません。

#### タイムシート

**投稿者ユーザーには、「承認用に送信」ボタンがあります**
