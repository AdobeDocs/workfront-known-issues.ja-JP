...
+++**（予定）2024年8月29日（PT）のメンテナンス更新**

### （予定）2024年8月29日（PT）のメンテナンス更新

#### カスタムフォーム

**Formsのデフォルトはプロジェクトフォーム**
...
**承認待ちウィジェットに削除されたドキュメントが表示される**

ユーザーがホームの保留中の承認ウィジェットを表示すると、削除済みのドキュメントが表示されます。ユーザーがこれらのドキュメントのいずれかをクリックすると、空白のページが表示されます。

#### ユーザー

**ユーザープロファイルの「電子メールのロケール」フィールドは無効になっています**

IMS 上の組織の場合、言語環境設定は各ユーザーのAdobe Experience Cloud プロファイルに保存されます。 Workfront Adobeプロファイルの「メールのロケール」フィールドは無効になっており（IMS 組織の場合のみ）、そのフィールドのツールチップに、ユーザープロファイルの言語設定にアクセスする方法の手順が表示されます。

これにより、管理者がユーザーのメールのロケール設定を変更しようとすると、英語に戻る問題が解決されます。
