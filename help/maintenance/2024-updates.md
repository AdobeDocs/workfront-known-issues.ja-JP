...
**レポートから一括の編集アイコンと削除アイコンが表示されない**

複数のレポート項目を一括で編集または削除しようとすると、編集アイコンと削除アイコンが表示されないことがあります。

**プロジェクト（財務データ）レポートにエラーが表示される**

プロジェクト（財務データ）レポートの詳細ページとマトリックスページの両方で、エラーが断続的に表示されます。

この問題の修正の一環として、このレポートタイプのプロジェクトは、データを読み込む前に自動的に再計算されなくなりました。 財務データレポートのデータを更新するには、ユーザーは個々のプロジェクトの財務を手動で再計算する必要があります。

+++

+++**2024年12月5日（PT）のメンテナンス更新**
...
「カテゴリ名」フィールドを含むレポートでリストを表示している場合、「カテゴリ名」列のコンテンツをクリックすると、対応するカスタムフォームを開くことができません。
...
**フォーム designer のネイティブフィールドに関する制限**

フォーム designer で作成されたカスタムフォームで、複数のネイティブフィールドがサポートされるようになりました。 以前は、フォーム別にネイティブフィールドが 1 つという制限がありました。
...
アップデートでユーザーがタグ付けされると、そのユーザーは、タグ付けを行った人物を確認できできません。 これは、「他の会社のユーザーは、次のユーザーのみを表示する必要があります…」という設定が発生します。 「自分の会社」に設定されます。
...
+++**2024年3月7日のWorkfront Fusion メンテナンスアップデート
