...
**「[!UICONTROL グループ]」エリアでスケジュールを開くと、空白のページに誘導される**

_設定_

ユーザーが「[!UICONTROL 設定]」エリアを開き、スケジュールを編集、またはコピーしようとすると、スケジュールが開かず、ユーザーに空白のページが表示されます。
...
**要求されたドキュメントを送信するためのリンクが、空白のページに誘導されるされる**
...
ユーザーがプロジェクトの「[!UICONTROL ドキュメント]」エリアでドキュメントの概要を開いてからタスクリストに移動し、タスクを選択して、タスクの概要を開こうとすると、タスクの概要は表示されず、次のメッセージが表示されます。
...
**[!UICONTROL 報告書内で割り当てをインライン編集できない]**

_報告書_

ユーザーが報告書内で割り当てをインライン編集しようとしても、フィールド値は変更されず、編集は保存されません。


+++

+++**2021年12月2日（PT）のメンテナンス更新**

**URL を手動で入力すると余分なスラッシュが追加される**

_要求_

ユーザーが要求に入力中に URL の手動入力を開始すると、URL の先頭付近に余分なスラッシュが追加されます。 ユーザーはスラッシュを削除できません。
...
このエラーはランダムに発生し、[!DNL Workfront] のエリアに影響する可能性があります。
...
**新しいキュートピックが選択されている場合、要求に添付されているカスタムフォームが更新されない**

_要求_

ユーザーが要求を作成する際、要求にカスタムフォームを自動的に添付するキュートピックを選択してから、別のキュートピックを選択すると、最初のキュートピックのカスタムフォームが要求に添付されたままになります。

**アイコンが正しく表示されない**

_[!DNL Workfront]_

アイコン画像が正しく表示されません。 この問題は、[!UICONTROL Workfront] の多くのエリアで報告されています。
...
このエラーはランダムに発生し、Workfront の任意のエリアに影響を与える場合があります。

+++

+++**2021年11月11日（PT）のメンテナンス更新**
...
**プロジェクトのドキュメントエリアに「[!UICONTROL プロジェクトフォルダー]」ドキュメントエリアにドキュメントヘッダーが見つからない**
...
+++**2021年11月4日（PT）のメンテナンス更新**

**新しいタスクがデフォルトで、間違ったタスク制約に設定される**

_タスク_

ユーザーが「[!UICONTROL 新規タスク]」ボタンを使用して新しいタスクを作成し、「新規タスクのデフォルトの開始日」オプションが「[!UICONTROL 今日]」に設定されている場合、作成したタスクのタスク制約は「[!UICONTROL 次の日以降に開始]」ではなく、「[!UICONTROL できるだけ早く]」に設定されます。
...
「[!UICONTROL 要求]」エリアの「[!UICONTROL 送信済みリスト]」で、要求名の左側にチェックボックスを追加しました。 これにより、要求の選択と追加情報の表示が容易になります。
...
[!UICONTROL ワークロードバランサー]エリアを「[!UICONTROL 期間]」でフィルター処理する際の、フィルター演算子を更新しました。
...
ユーザーが必須ラジオボタンフィールドを使用した要求を送信し、要求を送信する前にそのフィールドに値を選択しなかった場合、要求の送信時に最初の値が自動で選択されます。

+++

+++**2021年10月21日（PT）のメンテナンス更新**
...
**リストが画面の小さなエリアにのみ表示される**

_リスト_

ユーザーが [!DNL Mac] で [!DNL Big Sur OS] を使用して [!DNL Safari] ブラウザーを使用中にリストを表示しようとすると、リストは画面の小さなエリアにのみ表示されます。 ユーザーはリスト内をスクロールできますが、エリアが小さすぎて、リストを読みにくかったり、読めない場合があります。
...
新しい [!DNL Workfront] エクスペリエンスでユーザーが要求を作成し、「[!UICONTROL 割り当て]」フィールドに値を入力すると、ドロップダウンリストが表示されず、ユーザーは担当者の名前を選択できません。
...
ユーザーが「[!UICONTROL 通知]」エリアから[!UICONTROL すべてのお知らせ]ページを開いても、次のエリアにはお知らせが表示されません。
...
ユーザーが「[!UICONTROL 設定]」エリアを開き、リスト（[!UICONTROL カスタムフォーム]や[!UICONTROL アクセスレベル]など）を表示すると、そのリストの列ヘッダーがリストの列と揃いません。
...
ユーザーが、テキストカラーを変更する条件付き書式が設定された報告書を表示すると、テキストカラーは変更されない状態で表示されます。

+++

+++**2021年9月9日（PT）のメンテナンス更新**
...
リストまたは報告書にグループ化を適用した際、リストまたは報告書のヘッダーに「[!UICONTROL すべて折りたたむ、または展開する]」アイコンが表示されません。
...
**[!UICONTROL 新しい要求]ボタンが見つからない**

_要求_

新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーが[!UICONTROL 要求]ページに移動すると、「[!UICONTROL 新規要求]」ボタンが表示されず、ユーザーは要求を送信できません。
...
+++**2021年7月29日（PT）のメンテナンス更新**

**[!UICONTROL プルーフアクティビティ]ページと[!UICONTROL プルーフビューアの設定]ページが常に利用可能になる**

_[!DNL Workfront Proof]_

[!UICONTROL プルーフアクティビティ]および[!UICONTROL プルーフビューア]設定ページが、ユーザーがこれらのページにアクセスして更新する権限を持たない場合でも常に表示されるようになりました。
...
+++**2021年7月15日（PT）のメンテナンス更新**

**新しい要求のデフォルトの優先度が間違っている**

_要求_

新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーが要求を作成する際、[!UICONTROL プロジェクト環境設定]のデフォルトの優先設定が要求に反映されず、ユーザーは要求の送信前に優先度を調整することができません。
...
[!DNL Adobe Workfront Classic] でユーザーが[!UICONTROL ホーム]エリアからプロジェクト、タスク、または要求を作成する際、[!UICONTROL 割り当て先] typeahead フィールドにユーザー名が入力されません。

**時間の丸めが正しく行われない**

_タイムシート_

新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーが[!UICONTROL タイムシート]／[!UICONTROL すべてのタイムシート]に移動すると、一部のタイムシートでは合計時間数が 0.25 単位の増分ではなく 1 桁に丸められていますが、合計時間は個々のタイムシートで正しく表示されます。 たとえば、「すべてのタイムシート」エリアにはタイムシートの合計時間は「44.8」と表示されますが、タイムシートを開くと合計時間は「44.75」と表示されます。

**承認を委任できない**

_[!UICONTROL ホーム]_

[!DNL Adobe Workfront Classic] の「[!UICONTROL ホーム]」エリアでユーザーが「[!UICONTROL 自分の承認を委任]」をクリックし、委任先となるユーザーの名前の入力を開始すると、[!UICONTROL typeahead] リストに結果が表示されず、ユーザーを選択できません。

タスク報告書で&#x200B;**[!UICONTROL ガントチャート]ビューを使用できない**

_報告書_

メモ：この問題は、プロジェクト報告書にも影響を与えています。

新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーがタスク報告書を開くと、報告書のツールバーに[!UICONTROL ガントチャート]ビューを開くオプションが表示されません。 報告書のデフォルトで[!UICONTROL ガントチャート]ビューを表示するよう設定されている場合は、代わりに、リスト形式が表示されます。

**報告書で「[!UICONTROL 詳細を表示]」をクリックしても何も読み込まれない**

_ダッシュボード_

新しい [!DNL Adobe Workfront] エクスペリエンスのダッシュボードでユーザーが報告書を表示中に「[!UICONTROL 詳細を表示]」ボタンをクリックしても、何も起こらず、報告書内のすべての項目を表示することはできません。
...
**要求が空白の必須フィールドと共に送信される**

_要求_

ユーザーが要求を行って送信すると、必須フィールドの情報は要求と共に送信されません。
...
新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーがプロジェクトに添付されたカスタムフォームを展開しようとすると、カスタムフォームを開けず、エラーメッセージ「[!UICONTROL エラーが発生し、問題の解決に取り組んでいます。 作業を続行するには、このブラウザーページを更新してみてください。]」が表示されます。 ページを更新しても、問題は解決されません。
...
* [!DNL Adobe Workfront] ライオンがメインコンテンツエリアに追加されました。
...
新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーが要求を入力する際、送信された要求には、一部のカスタムフィールド（必須フィールドを含む）の入力値が含まれていません。
...
新しい [!DNL Adobe Workfront] エクスペリエンスでユーザーが要求を送信しようとしたときに、フィールドの文字制限を超えていると、要求を送信できず、エラーメッセージは表示されません。 [!DNL Adobe Workfront Classic] には、「[!UICONTROL [数値]文字オーバー」と表示され、要求を送信しようとすると、「次の点を確認してください：2,000 文字以内で入力してください (入力した文字数は[数値]文字です)]」というエラーメッセージが表示されます。

+++

+++**2021年6月10日（PT）のメンテナンス更新**
...
+++**2021年6月3日（PT）のメンテナンス更新**

**プロンプトによってページが揺れる**

_報告書_

新しい [!DNL Adobe Workfront] エクスペリエンスのユーザーがプロンプトを使用して報告書を実行すると、報告書内の列が左から右へとすばやく移動します。
...
[!DNL Workfront] Proof の次のエリアに、「[!UICONTROL ディアクティベート]」および 「[!UICONTROL 削除済み]」のユーザーラベルが追加されました。
...
新しい [!DNL Workfront] エクスペリエンスのユーザーがプロジェクトに移動し、「[!UICONTROL 概要]」エリアから[!UICONTROL 参照番号]をコピーしてから貼り付けると、数字の最後に「[!UICONTROL This]」が追加されます。
...
新しい [!DNL Workfront] エクスペリエンスでユーザーが特定のオブジェクトを開こうとすると、エラーメッセージ「[!UICONTROL （オブジェクト）は存在しません：web アドレスの入力ミスがある可能性があります。 アドレスを再度確認して、もう一度入力してください。]」 オブジェクトリンクは、リスト、最近使用したお気に入り、検索結果などに引き続き表示されますが、アクセスできず、削除されたオブジェクトを含むごみ箱には表示されません。



+++

+++**2021年5月20日（PT）のメンテナンス更新**
...
報告書の配信が予定されていても、ユーザーに報告書が届かないことがあります。
...
[!DNL Workfront for Outlook] 統合でユーザーがリッチテキストフィールドを更新する際、次の操作ができません。

* バックスペース
* テキストのコピー
* テキストの貼り付け
* すべてのフィールドに値が入力されたら要求を送信する

+++

+++**2021年5月6日（PT）のメンテナンス更新**
...
**リストおよび報告書にアーカイブ済みのプルーフが表示される**

_プルーフ_

ユーザーが[!UICONTROL ホーム]または[!UICONTROL マイ作業]で作業リストを表示すると、既にアーカイブされているプルーフがリストに表示されます。 アーカイブしたプルーフは、報告書とダッシュボードにも表示されます。
...
**でファイルが表示されない [!DNL Internet Explorer]**

_ドキュメント_

[!DNL Internet Explorer] ブラウザーを使用しているユーザーが オブジェクトの「[!UICONTROL ドキュメント]」エリアに移動すると、[!UICONTROL ドキュメント]画面が空白になり、ファイルは読み込まれません。 画面に一部のファイルが読み込まれる場合もありますが、表示されるファイルの数は「[!UICONTROL ドキュメント]」セクションの横に表示されている数と一致しません。

+++

+++**2021年4月22日（PT）のメンテナンス更新**
...
「[!UICONTROL 設定]」の「**[!UICONTROL チーム]」エリアに表示されないチームがある**

_[!UICONTROL 設定]_

管理者が「[!UICONTROL 設定]」の「[!UICONTROL チーム]」エリアに移動すると、自身が作成したチームのみが表示されます。 他の管理者が作成したチームは表示されません。
...
+++**2021年3月26日（PT）のメンテナンス更新**

**ビジネスケース内のボタンが正しく表示されない**

_プロジェクト_

ユーザーがビジネスケースを表示し、ウィンドウがフルスクリーンモードの場合、「[!UICONTROL 保存]」ボタンと「[!UICONTROL キャンセル]」ボタンが画面の中央付近に表示され、ビジネスケースの要素に重なって表示されます。

**報告書の並べ替えを変更できない**

_報告書_

新しい [!DNL Workfront] エクスペリエンスでユーザーが報告書の並び順を変更しようとすると、報告書の作成時に選択した並び順から変更されません。
...
ユーザーが[!DNL Workfront] APIの用語を使用して、[!DNL Workfront] モジュールの出力に含めるコレクションまたは参照フィールドを選択すると、そのモジュールの出力には、コロン（[!UICONTROL owner:name]など）を持つフィールドと、属性（名前は所有者の下のフィールド）も表示されます。 コロンでラベル付けされたフィールドにはデータが含まれておらず、シナリオのモジュールにマッピングすると、誤ったデータが提供されます。
...
「[!UICONTROL そのユーザーは、もう存在していません。 Web アドレスを間違って入力した可能性があります。 アドレスを再度確認して、もう一度入力してください。]」
...
メモ：この問題は、2021年2月26日に新しい [!DNL Workfront] エクスペリエンスで修正されました。
[!DNL Classic] エクスペリエンスで 2021年3月1日に修正されました。
...
+++**2021年2月25日（PT）のメンテナンス更新**

**[!UICONTROL スケジュール]ツールがどのエリアにもツールが読み込まれない**
...
**ユーザー報告書に一部のフィルターが適用されない**

_報告書_

新しい [!DNL Workfront] エクスペリエンスで、ユーザーが「[!UICONTROL 上位の親 ID]」フィールドを含むフィルターを使用してユーザー報告書を作成すると、報告書内の他のフィルタールールは適用されません。
...
+++**2021年2月18日（PT）のメンテナンス更新**

**「[!UICONTROL 要求]」エリアから不要なチェックボックスが削除されました**

_要求_

「[!UICONTROL 要求]」エリアの「送信済みリスト」で、要求名の左側のチェックボックスを削除しました。 このチェックボックスは機能とは関係がなかったので、わかりやすくするために削除しました。

**リンクからドキュメントにアクセスできない**

_ドキュメント_

新しい [!DNL Workfront] エクスペリエンスでユーザーが一部のドキュメントリンクをクリックすると、ドキュメントにアクセスできず、「[!UICONTROL ドキュメントが存在しません：web アドレスの入力ミスがある可能性があります。 アドレスを再度確認して、もう一度入力してください。]」 プルーフメール通知の[!UICONTROL 詳細を表示] リンクで、同じエラーが発生します。
...
+++**2021年2月11日（PT）のメンテナンス更新**

**選択したフォルダーに配達確認がアップロードされない**

_[!DNL Workfront Proof]_

ユーザーがフォルダーを開き、新しいプルーフを追加すると、プルーフはフォルダー内ではなく、オブジェクトの一般的な[!UICONTROL ドキュメント]エリアにアップロードされます。
...
+++**2021年2月4日（PT）のメンテナンス更新**

**書き出された報告書は [!DNL Workfront Classic] ブランドが表示される**

_報告書_

新しい Workfront エクスペリエンスでユーザーが報告書を書き出す場合、書き出された報告書に表示されるロゴは、[!DNL Workfront Classic] 設定（[!UICONTROL 設定]／[!UICONTROL システム]／[!UICONTROL ブランディング]）と一致します。
...
+++**2021年1月28日（PT）のメンテナンス更新**

**コメントに「[!UICONTROL 次の人物の代理]」と表示されない**

_更新_

[!DNL Workfront] 管理者が別のユーザーとしてログインし、オブジェクトの[!UICONTROL 更新]エリアでコメントに返信すると、ユーザー名の前に「[!UICONTROL 次の人物の代理]」というテキストが表示されません。

**ドキュメントを添付できない**

_要求_

新しい [!DNL Workfront] エクスペリエンスで、外部のドキュメントプロバイダーからの新しい要求にユーザーがドキュメントを追加しようとすると、[!UICONTROL ドキュメント]リストが読み込まれず、ユーザーはドキュメントを選択して要求を完了することができません。
...
ユーザーが「[!UICONTROL タイムシート]」エリアに移動すると、現在の週のタイムシートのみが表示されます。 今後数週間のタイムシートは自動的には生成されません。

+++

+++**2021年1月21日（PT）のメンテナンス更新**

**手動により列で並べ替えると、すべての結果が表示される**

_報告書_

新しい [!DNL Workfront] エクスペリエンスでユーザーが、報告書のグラフで棒をクリックしてから、列ヘッダーをクリックして、そのグループ化の結果を手動で並べ替えると、最初に選択したグループの結果だけでなく、すべての報告書結果が表示されます。
...
+++**2021年1月14日（PT）のメンテナンス更新**

**メール通知設定を更新できない**

_設定_

ユーザーがメール通知の設定を更新しようとすると、「[!UICONTROL メール通知]」エリアにアクセスできず、「[!UICONTROL もう一度試してください。 エラーが 発生しました。 [!DNL Workfront] にお問い合わせください。問題の原因を確認して修正させていただきます。]」というエラーメッセージが表示されます。

**[!UICONTROL ガントチャート]によって一部のフィールドが切り捨てられる**

_リスト_

ユーザーが一部のリストエリア（[!UICONTROL 説明]など）で[!UICONTROL ガントチャート]を開くと、テキストが切り捨てられます。 全文を表示するには、フィールドをダブルクリックする必要があります。
